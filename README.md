...
- Progress 推进：一键 `+1`，记录每一步推进；推进后的 Active 任务会移动到列表底部，避免底部任务长期看不见
...
- 专长选择采用二次确认：先点选专长，再点击确认；也可以稍后在 `/build` 页面选择
...
- 职业共鸣：普通触发 1 秒右侧轻量动画，首次发现触发新共鸣发现弹窗
...
- Tailwind CSS
- Framer Motion
- Zustand（persist 中间件，版本迁移 v1→v13）
