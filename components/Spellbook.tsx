...
export function Spellbook({ onClose }: { onClose: () => void }) {
  ...
  return (
    ...
    >
      ...
      <motion.div
        className="relative flex h-[88vh] w-full max-w-6xl flex-col overflow-hidden rounded-2xl border border-slate-200 bg-white shadow-2xl dark:border-slate-700 dark:bg-slate-900"
      ...
      >
        ...
        <div className="border-b border-slate-200 px-5 py-4 pr-12 dark:border-slate-700">
        ...
        <div className="flex min-h-0 flex-1 flex-col md:flex-row">
          <aside className="border-b border-slate-200 bg-slate-50/80 dark:border-slate-700 dark:bg-slate-950/70 md:w-[320px] md:shrink-0 md:border-b-0 md:border-r">
                  ...
                  return (
                    <button
                      ...
                      className={`focus-ring min-h-[72px] rounded-xl border px-3 py-2 text-left transition ${
                        isSelected
                          ? `${meta.borderColor} ${meta.bgColor} shadow-sm dark:bg-slate-800/80`
                    ...
                    >
                      <div className="flex items-center gap-2">
                        ...
                        <div className="min-w-0 flex-1">
                          <div className={`truncate text-sm font-bold ${isSelected ? meta.color : "text-slate-800 dark:text-slate-100"}`}>
                            {cn}
                          </div>
                          <div className="truncate text-[11px] text-slate-500 dark:text-slate-400">{meta.label}</div>
                        </div>
                      </div>
                      <div className="mt-2 flex items-center justify-between gap-2 text-[11px] text-slate-500 dark:text-slate-400">
                        ...
                        <span className={cs.scrolls > 0 ? "font-bold text-amber-700 dark:text-amber-300" : ""}>
                          📜 {cs.scrolls}
                        </span>
                      </div>
                    </button>
          ...
          <section className="min-h-0 flex-1 overflow-y-auto p-5 dark:bg-slate-900">
            <div className={`rounded-xl border-2 p-4 ${selectedMeta.borderColor} ${selectedMeta.bgColor} dark:bg-slate-800/80`}>
                ...
                <div className="flex items-center gap-2">
                  {selectedState.scrolls > 0 && (
                    <button
                      ...
                      className="focus-ring inline-flex items-center gap-1.5 rounded-lg border border-amber-300 bg-amber-100 px-3 py-1.5 text-xs font-bold text-amber-800 transition hover:bg-amber-200 dark:border-amber-500/50 dark:bg-amber-500/20 dark:text-amber-200 dark:hover:bg-amber-500/30"
                    >
                      <Scroll size={14} />
                  ...
                  {selectedState.scrolls === 0 && (
                    <span className="rounded-lg border border-white/70 bg-white/60 px-3 py-1.5 text-xs text-slate-400 dark:border-slate-700 dark:bg-slate-950/60 dark:text-slate-400">
                      无卷轴
                    </span>
                  )}
                </div>
              ...
              <div className="mt-4 h-2 overflow-hidden rounded-full bg-white/60 dark:bg-slate-950/60">
                <motion.div
                ...
                return (
                  <div
                    ...
                    className={`rounded-lg border p-3 transition ${
                      isOwned
                        ? "border-slate-200 bg-white shadow-sm dark:border-slate-700 dark:bg-slate-800/80"
                        : "border-dashed border-slate-300 bg-slate-100 opacity-50 dark:border-slate-700 dark:bg-slate-950/70"
                    }`}
                  >
                    <div className="flex items-center justify-between gap-2">
                      <div className="flex min-w-0 items-center gap-1.5">
                        <span className="text-lg">{line.emoji}</span>
                        <span className={`truncate text-sm font-semibold ${isOwned ? "text-slate-900 dark:text-slate-100" : "text-slate-400 dark:text-slate-500"}`}>
                          {isOwned ? currentSkillName : "???"}
                        </span>
                        {isOwned && currentTier > 1 && (
                          <span className="truncate text-xs text-slate-400 dark:text-slate-500">（{line.name}）</span>
                        )}
                      </div>
                      {isOwned && (
                        <span className="shrink-0 rounded-full bg-violet-100 px-2 py-0.5 text-[10px] font-bold text-violet-700 dark:bg-violet-500/20 dark:text-violet-200">
                            ...
                            {Array.from({ length: MAX_TIER }).map((_, t) => (
                              <div
                                ...
                                className={`h-1.5 w-2 rounded-full ${
                                  t + 1 <= currentTier ? "bg-violet-400" : "bg-slate-200 dark:bg-slate-700"
                                }`}
                              />
                            ))}
                          </div>
